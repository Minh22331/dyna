    ...
    float cellIndex)
{
    ...
    float clusterBaseHalfWidth = max(4.0, cellWidth * lerp(0.12, 0.22, Hash11(cellIndex * 23.731 + 9.27)) * widthScale);
    float clusterHalfWidth = max(4.0, clusterBaseHalfWidth * taper);
    ...
    // 子光束共享一个上游虚拟光源，并沿传播方向逐渐展开。
    // 宽度只做轻微收缩，避免继续跟随大光束 envelope 收束成平行尖线。
    float fanProgress = smoothstep(0.05, 0.85, rayProgress);
    float fanSpread = lerp(0.30, 1.00, fanProgress);
    float subWidthTaper = lerp(0.95, 0.72, fanProgress);
    float subBaseWidth = max(1.0, clusterBaseHalfWidth * lerp(0.18, 0.34, Hash11(cellIndex * 37.91 + 1.77)) * subWidthTaper);
    float subFeather = max(2.0, subBaseWidth * lerp(1.0, 2.2, softness));

    float sub0 = CalculateGodRayBeamMask(
        rayCoord,
        centerCoord - clusterBaseHalfWidth * fanSpread * lerp(0.52, 0.86, Hash11(cellIndex * 5.13 + 1.0)),
        subBaseWidth * 0.78,
        subFeather);

    float sub1 = CalculateGodRayBeamMask(
        rayCoord,
        centerCoord - clusterBaseHalfWidth * fanSpread * lerp(0.06, 0.28, Hash11(cellIndex * 7.71 + 2.0)),
        subBaseWidth * 1.08,
        subFeather * 1.15);

    float sub2 = CalculateGodRayBeamMask(
        rayCoord,
        centerCoord + clusterBaseHalfWidth * fanSpread * lerp(0.08, 0.34, Hash11(cellIndex * 17.3 + 5.0)),
        subBaseWidth * 0.92,
        subFeather);

    float sub3 = CalculateGodRayBeamMask(
        rayCoord,
        centerCoord + clusterBaseHalfWidth * fanSpread * lerp(0.48, 0.82, Hash11(cellIndex * 23.2 + 7.0)),
        subBaseWidth * 0.70,
        subFeather * 1.25);
